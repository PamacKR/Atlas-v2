...
The MCP server also exposes the existing in-app readiness report, deterministic named-material resolution with explicit found/ambiguous/not-found states, full synced Classroom announcement/assignment content, General/unsorted agent-note creation, parent-resource metadata for document-page search hits, and bounded PDF/image visual reads through Atlas ids. It remains a local data bridge only: Atlas does not call an AI service and does not accept arbitrary file paths from the agent.
