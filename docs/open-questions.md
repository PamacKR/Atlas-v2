...
The user's concrete spec for the Drive adapter (`ROADMAP.md` Phase 3, now reordered to build Drive first):
- Scans one designated Drive folder — on every launch, plus polling every ~15–30s while Atlas is running (the user explicitly accepted this over true push, which would need a public HTTPS endpoint — not workable for a local desktop app).
- New files trigger a review panel: course + Resource/Note assignment **per file**, selectable individually or in bulk, with an explicit "do this later" dismiss and a persistent way to reopen the pending list (not forced to resolve immediately).
- Imported files are downloaded and copied into local managed storage exactly like any other resource/note — Drive is the inbox, not a live remote reference; "Atlas owns the data" (`CLAUDE.md`) still holds once something's tagged.
- Still open: whether a file tagged "Note" and typed (`.md`/`.txt`) becomes a typed note (content = file text) versus a file tagged "Note" that's a scan (PDF/image) becomes a handwritten note via the same path `importScanFileIntoNote` already uses — likely both, branching on file type same as manual import does, but not yet built.

**Status:** Open, actively being built as the reordered start of Phase 3 — see `ROADMAP.md`. Blocked on the user creating their own free-tier Google Cloud OAuth client (`docs/open-questions.md` #7 already settled that this is bring-your-own, no Atlas-shipped credential).
