...
- **Root cause (confirmed, not guessed)**: `syncClassroomCourseworkForMappedCourses()` looped over every mapped course with no per-course `try`/`catch` — one course's `courseWork.list`/`announcements.list` call throwing (expired token, transient API error, anything) silently aborted the sync for *every other* mapped course too, and the only catch was the outer `scanClassroomAndNotify()`, which just `console.error`'d and returned `false` — indistinguishable in the UI from "ran fine, nothing new." Fixed: each mapped course's `courseWork`/`announcements`/`courseWorkMaterials` calls are now wrapped individually; failures are collected as `{courseId, courseName, message}` and returned up through `scanClassroomAndNotify`/`classroom:syncNow`/both mapping handlers/the new per-course connect handler, and the renderer now `alert()`s the user when a sync partially fails instead of it looking identical to success.
...
- **Course-detail Announcements/Assignments/Classwork**: three new inline sections (same visual idiom as the existing Deadlines/Watched-folders sections, not a new tab pattern — course detail had never used tabs before this), shown only once a course is actually linked.
