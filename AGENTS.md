...
**Starting a fresh session (new chat, new agent, or after context compaction)?** Read, in order: this file, then [`STATUS.md`](STATUS.md) (what's actually been decided/built and what's pending), then [`open-questions.md`](open-questions.md), then [`DESIGN.md`](DESIGN.md) for the active UI-overhaul contract. If the task will query or mutate Atlas through MCP, also read [`MCP_AGENT_GUIDE.md`](MCP_AGENT_GUIDE.md) before using the MCP tools. Don't re-derive decisions already recorded in those files or in `ARCHITECTURE.md`/`ROADMAP.md` — treat them as settled unless the user says otherwise. `STATUS.md` is the one document expected to go stale fastest; update it whenever real progress happens, not just at the end of a session.
...
| `MCP_AGENT_GUIDE.md` | The user gives a standing instruction about how an external agent should retrieve, clarify, summarize, or write Atlas data through MCP. | An MCP task ignores the bounded-search, ambiguity, or no-invention rules recorded there. |
...
**Added 2026-07-29 at the user's explicit instruction.** Phase 6 was a complete UI overhaul, not a series of improvements. It was completed for Atlas's personal-use scope on `ui-overhaul-v2` and later merged into `main`. Public-release polish is intentionally deferred. Do not make unplanned cosmetic changes; future visual work should be scoped explicitly and still follow the mockup contract below:
