...
- [x] Search dead-click fix: every result type (notes, files, page/slide/sheet hits, announcements, assignments) now opens something when clicked. See `STATUS.md` (2026-07-29).
- [x] Search ordering: courses first, then file/note names, then in-file content, then Classroom items — hard-partitioned so a strong content match can never outrank a weak name match. See `STATUS.md` (2026-07-29).
- [ ] **Search overhaul (remaining)** — page-level hits collapsed under their parent file so 3,000+ indexed pages can't flood the list, source badges, a course filter, visual section headers marking the four groups above in the results dropdown. The headers/collapsing are UI changes, held back by the no-UI-patching rule until the Phase 6 redesign. See `phase5-spec.md` §2.
