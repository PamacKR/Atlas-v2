...
(async () => {
  ...
  if (Math.abs(expandedBrandGeometry.logo.x - expandedBrandGeometry.navIcon.x) > 1) {
    throw new Error(`FAIL: expanded logo is not centered on the navigation icon rail: ${JSON.stringify(expandedBrandGeometry)}`);
  }
  ...
  if (Math.abs(collapsedBrandGeometry.logo.x - collapsedBrandGeometry.navIcon.x) > 1) {
    throw new Error(`FAIL: collapsed logo is not centered on the navigation icon rail: ${JSON.stringify(collapsedBrandGeometry)}`);
  }
  await window.reload();
  await window.waitForTimeout(500);
