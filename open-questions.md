...
- Scans multiple enabled Drive folders (each pasted as a link, not picked via a Google Picker widget) — on every launch, plus polling every 20s while Atlas is running. Each source can be paused, removed, or given a default Atlas course. **Recursively scans subfolders too**, and combines all sources into one review inbox.
...
- Resource imports now keep the original in Drive by default and store a remote Drive reference plus extracted text in Atlas; the review popup lets the user choose **Keep in Drive** or **Save local copy** per file or in bulk. Only Note imports still need the local buffer-copy path. Remote extraction fetches bytes temporarily and discards them. Existing local Drive copies are preserved, while missing ones are migrated to remote references instead of being deleted and re-queued.
...
**Status:** Resolved — built and manually verified against the user's real Google account and Drive folder (not covered by the automated `scripts/verify-app.js` suite, which has no way to supply a live OAuth token). On 2026-09-04 the scanner was extended to try the college Classroom OAuth client first, then the personal Drive client, and redesigned around multiple sources, launch-time review, and remote-by-default Resource imports.
